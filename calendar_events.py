...
FILTERED_SUBJECTS = [
    ...
    #"ADI Team Maverick Daily Standup (OV-2)",
    "Focus time",
    "Team Maverick Lunch",
    "Add Comments to tickets"
]
...
def main():
    ...
    output = ""
    with open(r"C:\Users\sjones\Downloads\test_calendar.CSV", "r", encoding='utf-8') as csv_file, \
        open(r"C:\Users\sjones\Downloads\daily_tasks\\" + filename, "w") as output_file:
        ...
        for row in todays_events:
            ...
            if "Gritters" in row["Meeting Organizer"] or "Standup" in row['\ufeff"Subject"'] or "Madeleine" in row["Required Attendees"]:
                row["Start Time"] = row["Start Time"] - timedelta(hours=1)
                row["End Time"] = row["End Time"] - timedelta(hours=1)
            if row["Start Time"].hour == 11:
                no_lunch = True
        if not no_lunch:
            todays_events.append({'\ufeff"Subject"': "Lunch",
