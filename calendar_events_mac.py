...
def main():
    ...
    output = ""
    with (open(r"/Users/sjones/Downloads/calendar.csv", "r", encoding='utf-8') as csv_file, open(f"/Users/sjones/Downloads/daily_tasks/{filename}", "w") as output_file):
        ...
        for row in todays_events:
            ...
            output = write_out(f"- [ ] {start_time} {subject} \n- [ ] {end_time}", output_file, output)
...
def handle_lunch(todays_events):
    ...
    if not no_lunch:
        todays_events.append(
                    {'Event Title': "Lunch",
                    "Start Time": datetime(day=TODAY.day, month=TODAY.month, year=TODAY.year, hour=11, minute=0, tzinfo=timezone(-timedelta(hours=6))),
                    "End Time": datetime(day=TODAY.day, month=TODAY.month, year=TODAY.year, hour=11, minute=30, tzinfo=timezone(-timedelta(hours=6)))}
                )
